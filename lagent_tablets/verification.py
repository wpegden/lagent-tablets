...
# Note: this doesn't mask comments/strings like the supervisor does, but catches most cases
FORBIDDEN=$(grep -n -E '\\b({forbidden_pattern})\\b' "$LEAN_FILE" | grep -v '^[0-9]*:--' || true)
if [ -n "$FORBIDDEN" ]; then
    echo "WARNING: Potential forbidden keywords (supervisor will do precise check):"
    echo "$FORBIDDEN"
fi

# Delete .olean and compile
OLEAN_CANDIDATES=(
    "$REPO/.lake/build/lib/Tablet/$NAME.olean"
    "$REPO/build/lib/Tablet/$NAME.olean"
)
for OLEAN in "${{OLEAN_CANDIDATES[@]}}"; do
    [ -f "$OLEAN" ] && rm -f "$OLEAN"
done
...
SORRY_WARNINGS=$(echo "$BUILD_OUTPUT" | grep -i sorry || true)
...
def write_scripts(
    ...
    *,
    ...
    """Write check_node.sh and check_tablet.sh to the state_dir/scripts/ directory."""
    scripts_dir = state_dir / "scripts"
    scripts_dir.mkdir(parents=True, exist_ok=True)

    check_node = scripts_dir / "check_node.sh"
    check_node.write_text(
        generate_check_node_sh(repo_path, state_dir, allowed_prefixes=allowed_prefixes, forbidden_keywords=forbidden_keywords),
        encoding="utf-8",
    )
