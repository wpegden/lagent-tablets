...
- Situation: Correspondence verification including preamble items, provenance excerpts, previous results, and multiple changed nodes.
...
- **Preamble** (env: definition, difficulty: hard, paper ref: none) — imports: none
...
=== PREAMBLE INTERFACE ITEMS TO CHECK ===
...
- **Preamble[1]** (definition) [Ambient conventions]
The paper works over a fixed finite universe.
- **Preamble[2]** (proposition) [Imported background fact]
A finite combinatorial bound from Mathlib is used without proof.

=== CHANGE CONTEXT FOR THE LOCAL NODES THAT REOPENED THIS FRONTIER ===
...
--- main_result_part_a (last verified in cycle-3) ---
Previous Lean statement/import block:
...
theorem main_result_part_a : True := by

Current Lean statement/import block:
import Tablet.Preamble
import Tablet.key_lemma
...
Previous NL statement block:
\begin{theorem}[Main result, part A]
\label{thm:main}
This node states the coarse first half of the main theorem.
\end{theorem}

Current NL statement block:
\begin{theorem}[Main result, part A]
\label{thm:main}
This node states the sharpened first half of the main theorem under an explicit hypothesis.
\end{theorem}

--- main_result_part_b (last verified in cycle-3) ---
Previous Lean statement/import block:
import Tablet.Preamble
...
theorem main_result_part_b : True := by
...
\label{thm:main}
This node states the second half of the main theorem.
\end{theorem}
...
=== PREVIOUS CYCLE'S VERIFICATION RESULTS ===

The worker was asked to fix these issues. Check whether each fix is genuine or superficial. Do NOT approve a node just because the worker claims to have fixed it — verify independently.

- **main_result_part_a** (correspondence): [previous correspondence-agent finding from the prior cycle]
- **main_result_part_b** (paper_faithfulness): [previous correspondence-agent finding from the prior cycle]

=== YOUR RESPONSE ===

Write your assessment as JSON to the raw file `/EXAMPLE_PROJECT/.agent-supervisor/staging/correspondence_result_2.raw.json`:
...
MANDATORY:
1. Write the JSON to `/EXAMPLE_PROJECT/.agent-supervisor/staging/correspondence_result_2.raw.json`.
2. Run `python3 /EXAMPLE_PROJECT/.agent-supervisor/scripts/check.py correspondence-result /EXAMPLE_PROJECT/.agent-supervisor/staging/correspondence_result_2.raw.json`.
3. If that passes, write the completion marker `/EXAMPLE_PROJECT/.agent-supervisor/staging/correspondence_result_2.done` and stop.

The supervisor will rerun the same checker and then write the canonical result file `/EXAMPLE_PROJECT/correspondence_result_2.json`.
```
