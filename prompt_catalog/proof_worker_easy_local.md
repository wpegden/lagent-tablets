...
- Situation: Proof-formalization worker on an easy local node with prior invalid feedback and targeted paper focus.
- Bracketed placeholders in this file stand for dynamic runtime text from agents, humans, or policy injection:
  ...
  - `[Lean build output from the prior invalid attempt]`
...
YOUR ROLE: **Worker** (proof_formalization phase, EASY node). You are proving `bound_corollary` using ONLY its existing children. No new imports, no new files.
...
--- HUMAN FEEDBACK (received at cycle 3, 1 cycle ago) ---
[human feedback entered through the viewer]
...
Reviewer's assessment: local proof work is still possible

PREVIOUS CYCLE: INVALID -- [deterministic validation blocker from the prior attempt]
Build error output:
```
[Lean build output from the prior invalid attempt]
```

Fix the issue and try again. Rerun the deterministic self-checks before handing off.
=== Active Node: bound_corollary ===
Env: corollary
Status: open
Title: Explicit bound
Paper reference: lines 19-23; label=cor:bound

--- bound_corollary.lean ---
import Tablet.Preamble
import Tablet.main_result_part_b

-- [TABLET NODE: bound_corollary]
-- Do not rename or remove the declaration below.

theorem bound_corollary : True :=
sorry
...
Read `Tablet/bound_corollary.tex` and any other `.tex` files for NL context.
...
|------|-----|--------|------------|-----------|-------|---------|
| bound_corollary | corollary | open | easy | lines 19-23; label=cor:bound | Explicit bound | Preamble, main_result_part_b |
...
[Lines 19-23] explicit bound statement
...
YOUR ACTIVE NODE: `bound_corollary`
YOUR SINGLE GOAL: Prove `Tablet/bound_corollary.lean` using ONLY its existing imports and children.
...
WORKFLOW:
1. Work ONLY on `Tablet/bound_corollary.lean`, and only on the proof body after `:=`. Do NOT edit `Tablet/bound_corollary.tex`, `Preamble.lean`, or any other node's files.
2. When you have a result -- whether the proof compiles or you're stuck -- STOP and write the raw handoff file `/EXAMPLE_PROJECT/.agent-supervisor/staging/worker_handoff.raw.json`.
3. Do NOT move on to other nodes. The reviewer decides what to work on next.

You may:
- Edit the proof body (everything after `:=`) in `Tablet/bound_corollary.lean`

You must NOT:
- Add or remove any `import` lines (the existing imports are all you need)
- Create any new files (no new helper nodes)
- Edit `Tablet/bound_corollary.tex`, `Tablet/Preamble.lean`, or any other node's files
- Modify the declaration line (`theorem bound_corollary ...` -- this is frozen)
...
MANDATORY BEFORE SUBMITTING: Run the self-check and fix any errors:
  ...
  python3 /EXAMPLE_PROJECT/.agent-supervisor/scripts/check.py tablet /EXAMPLE_PROJECT
  python3 /EXAMPLE_PROJECT/.agent-supervisor/scripts/check.py node bound_corollary /EXAMPLE_PROJECT
