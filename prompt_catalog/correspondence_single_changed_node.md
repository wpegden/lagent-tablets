...
- Situation: Correspondence verification with old-vs-new context for one node that reopened the frontier.
...
=== CHANGE CONTEXT FOR THE NODE THAT REOPENED THIS FRONTIER ===
...
Write your assessment as JSON to the raw file `/EXAMPLE_PROJECT/.agent-supervisor/staging/correspondence_result_1.raw.json`:
...
MANDATORY:
1. Write the JSON to `/EXAMPLE_PROJECT/.agent-supervisor/staging/correspondence_result_1.raw.json`.
2. Run `python3 /EXAMPLE_PROJECT/.agent-supervisor/scripts/check.py correspondence-result /EXAMPLE_PROJECT/.agent-supervisor/staging/correspondence_result_1.raw.json`.
3. If that passes, write the completion marker `/EXAMPLE_PROJECT/.agent-supervisor/staging/correspondence_result_1.done` and stop.

The supervisor will rerun the same checker and then write the canonical result file `/EXAMPLE_PROJECT/correspondence_result_1.json`.
```
