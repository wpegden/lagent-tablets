...
- Situation: Theorem-stating worker in broad mode with reviewer guidance, open blockers, support actions, and a preserved invalid retry.
- Bracketed placeholders in this file stand for dynamic runtime text from agents, humans, or policy injection:
  ...
  - `[reviewer decision about an unsupported node from the previous cycle]`
...
[Lines 14-23] current configured target frontier
...
Theorem-stating continues until this list is empty. Resolve these items before treating the tablet as complete.
- [correspondence] main_result_part_a: The Lean statement is still weaker than the paper statement.

--- TARGET-SUPPORT ACTIONS ---
Resolve these unsupported nodes before treating the target-support DAG as complete.
- [remove] floating_note: [reviewer decision about an unsupported node from the previous cycle]

--- PREVIOUS ATTEMPT ---
The previous attempt on this in-flight cycle was INVALID. You are now on attempt 3.
Blocker: [deterministic validation blocker from the prior attempt]
The invalid attempt's worktree has been preserved for you to continue from.
Consecutive INVALID attempts on this in-flight cycle: 2.
