...
- Treat disguised definitions as a structural modeling problem. If a theorem/lemma/corollary node is really being used to introduce a concept, prefer reshaping it into a genuine definition node (or a documented imported definition in `Preamble.tex`) instead of accepting the package as-is.
