...
- It is acceptable for one configured target paper item to be covered collectively by multiple non-`helper` nodes. Do not insist on one node per target, but do insist that the covering set together fully captures the target item.
